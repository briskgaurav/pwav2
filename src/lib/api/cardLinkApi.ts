/**
 * Card-link API calls — VC ↔ UC linking flow.
 *
 * Every function returns `CardLinkStateResponse`. Callers feed the result
 * into `useCardLinkJourney.call()` which dispatches it into Redux.
 * Screen routing happens automatically via `response.nextAction.code`.
 */
