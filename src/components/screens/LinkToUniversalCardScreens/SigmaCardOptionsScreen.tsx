...
export default function SigmaCardOptionsScreen({ universalCards, onNext }: Props) {
    const dispatch = useAppDispatch()
    const handleSetPendingLinkUniversalCardId = (id: string | null) => dispatch(setPendingLinkUniversalCardId(id))

    // For now, treat all fetched universal cards as unlinked
    const unlinkedUniversalCards = universalCards

    const linkedUniversalCards: UniversalCard[] = []
                        ...
                        <>
                            ...
                            {unlinkedUniversalCards.map((card) => (
                                <button
                                    ...
                                    onClick={() => setSelectedCard(universalCardStableId(card))}
                                ...
                                >
                                    <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${selectedCard === universalCardStableId(card) ? 'border-orange bg-orange' : 'border-text-primary/40'
                                        }`}>
                                    ...
                                    <span className="text-sm text-text-primary">
                                        {maskCardNumber(card.ucPanMasked ?? card.maskedCardNumber ?? '')} ( Universal Card )
                                    </span>
                                </button>
                            ))}
                        </>
