...
function getFilterLabel(filters: CardFilterType[]): string {
  if (filters.includes('all') || filters.length === 0) return 'All Cards'
  if (filters.length === 1) return `${FILTER_LABELS[filters[0]]} Cards`
  if (filters.length === 2) return `${FILTER_LABELS[filters[0]]}, ${FILTER_LABELS[filters[1]]}`
  return `${filters.length} Selected`
}

interface ScanPayFilterBarProps {
