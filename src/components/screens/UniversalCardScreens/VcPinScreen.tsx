...
import { verifyVcPin } from '@/lib/api/cardLinkApi';
...
/**
 * VC PIN entry screen — driven by `nextAction.code === 'VERIFY_VC_PIN'`.
 *
 * The user enters their existing VC PIN (4–6 digits).
 * On success the backend transitions to the next state automatically.
 * Server-side lockout policy applies — do not gate locally.
 */
...
export default function VcPinScreen() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await call(() => verifyVcPin(requestId, pin));
    } catch (err: unknown) {
      const message =
  ...
  return (
    <div className="flex-1 flex flex-col">
      <div className="flex-1 flex flex-col items-center justify-start p-6 py-10 text-center gap-4">
        <h2 className="text-xl font-semibold text-text-primary">Verify Virtual Card PIN</h2>
        <p className="text-sm text-text-secondary">
          Enter the PIN for your Virtual Card
        </p>
        {vcMasked && (
